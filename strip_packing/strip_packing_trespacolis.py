...
def build_rect_strip_packing_model_Tres(instance_id: str):
    """
...
def build_rect_strip_packing_model_altered_Tres(instance_id: str):
    ...
    # Precompute the Bounds for each (i,j)
    def ub_ij_init(m, i, j):
        return min(pyo.value(m.max_length) - pyo.value(m.rect_length[i]), pyo.value(m.rect_length[j]))
    m.upper_bound1 = pyo.Param(m.overlap_pairs, initialize=ub_ij_init, mutable=False)

    def lb_ij_init(m, i, j):
        return max(- pyo.value(m.rect_length[i]), - pyo.value(m.max_length) + pyo.value(m.rect_length[j]))
    m.lower_bound1 = pyo.Param(m.overlap_pairs, initialize=lb_ij_init, mutable=False)
    ...
    def i_above_j_rule(disjunct, i, j):
        ...
        disjunct.cons.add(m.x[j] + m.upper_bound1[i,j] >= m.x[i])
        disjunct.cons.add(m.x[i] - m.lower_bound1[i,j] >= m.x[j])
    m.i_above_j = Disjunct(m.overlap_pairs, rule=i_above_j_rule)
...
if __name__ == "__main__":
    # model = build_rect_strip_packing_model_altered_Tres("5_1")
    model = build_rect_strip_packing_model_Tres("5_1")
    # model = build_rect_strip_packing_model_reagg_Tres("5_1")
